...
public enum ApplicationCF implements CFEnum {
  ...
  /**
   * each row models name:value pairs. {@see org.usergrid.persistence.Schema}
   * for the list of dictionary types
   */
	ENTITY_DICTIONARIES("Entity_Dictionaries", "BytesType"),

  /**
   * Rows that are full of UUIDs. Used when we want to have a row full of references to other entities.
   * Mainly, this is for collections. Collections are represented by this CF.
   */
	ENTITY_ID_SETS("Entity_Id_Sets", "UUIDType"),

  /**
   * Typed vs. untyped dictionary. Dynamic entity dictionaries end up here.
   * {@link EntityManagerImpl#getDictionaryAsMap(org.usergrid.persistence.EntityRef, String)}
   */
